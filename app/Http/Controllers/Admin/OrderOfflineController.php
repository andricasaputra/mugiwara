...
class OrderOfflineController extends Controller
{
    ...
    {
        $accomodation = Accomodation::with('room')->latest()->get();
        //$rooms = Room::where('accomodation_id', $accomodation->id)->get();
        $tax = Setting::where('type', 'tax')->where('is_active', 1)->latest()->first();

        return view('admin.order_offline.create')->withAccomodation($accomodation)->withTax($tax);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect(route('admin.order.offline.index'))->withSuccess('Berhasil Tambah Order');

        } catch (\Exception $e) {

            DB::rollback();

            return redirect(route('admin.order.offline.index'))->withErrors('Gagal Tambah Order, Error :' . $e->getMessage());
    ...
    public function detail(OrderOffline $order)
    {
        ...
        return view('admin.order_offline.detail')->withOrder($order);
    }
    ...
    public function checkIn(Request $request)
    {
        ...
        try {
            ...
            return redirect(route('admin.order.offline.index'))->withSuccess('Berhasil checkin tamu ruangan nomor ' . $order->room?->room_number);

        } catch (\Exception $e) {

            DB::rollback();

            return redirect(route('admin.order.offline.index'))->withErrors('Gagal checkin ruangan, error ' . $e->getMessage());
    ...
    public function checkOut(Request $request)
    {
        ...
        try {
            ...
            return redirect(route('admin.order.offline.index'))->withSuccess('Berhasil checkout tamu ruangan nomor ' . $room->room_number);

        } catch (\Exception $e) {

            DB::rollback();

            return redirect(route('admin.order.offline.index'))->withErrors('Gagal checkout ruangan, error ' . $e->getMessage());
        }
    }

    public function getRoomNumber(Request $request)
    {
        $room->where('accomodation_id', $request->accomodation_id);
    }
